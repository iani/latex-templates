\documentclass[a4paper,11pt]{article}
\usepackage{fontspec}
\usepackage{hanging}
\setmainfont{Linux Libertine O} % libertine definitely better than Times New Roman
% \setmainfont{Times New Roman}
\usepackage[left=2cm, right=2cm, top=2.5cm, bottom=2.5cm]{geometry}
% \usepackage{ulem} %This is needed to enable underlined emphasis
% (e.g. _underlined_)
\usepackage{eurosym}
\usepackage{hyperref}

% Customeheaders
% \usepackage{fancyhdr}
% \usepackage{lastpage}
% \pagestyle{fancy}
% \rhead{GiMiC B1.2}
% \lhead{Esther Lemi-Petropoulou}
% \lfoot{EP 7.4.3}
% \rfoot{\pageref{LastPage} pages}

\sloppy

%Title
\title{
  \begin{LARGE}
    \textbf{Proposal for Inter\_agency Artistic Research Project\\}
    \textbf{Duo for Violoncello and Sensor-Controlled SuperCollider (``Zairja'')\\ }
    \textbf{Submitted by: Iannis Zannos}
  \end{LARGE}
}

% \date{}

\begin{document}
  
\maketitle

%\vspace{3cm}

% \indent
% Principal Investigator: Lemi-Petropoulou, Esthir

% \vspace{0.1cm}

% Scientific Area: Humanities and Arts

% \vspace{0.1cm}

% Scientific Field 1: Arts

% \vspace{0.1cm}

% Scientific Field 2: Performing Arts Studies

% \vspace{0.1cm}

% Project’s Duration (in months) 24

% \vspace{0.1cm}

% Total Budget (\euro): 142560 \euro

% \vspace{0.1cm}

% Host Institution: Department of Audio and Visual Arts, Ionian University 

% \vspace{1cm}

% Cooperative Organisations:

% \vspace{0.2cm}

% 1. Lisboa Incomum / Dies de Musica Electroacustica, Portugal

% \vspace{0.1cm}

% 2. Garage 21 Performing Arts Center, Corfu, Greece

% \newpage

\input{body.tex}

\end{document}
